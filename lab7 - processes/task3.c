//  task3.c - Test by entering "/bin/ls -alr &" with CTRL-D
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc,char *argv[])
{
    ...
    // number of spaces in input string + 1 = # of Arguments
    arguments = arguments + 1;
    
    // Check whether the input string contains the character "&"  FOR TASK3
    char *charDectected = strchr(command,'&');  // charDectected=NULL if THERE IS NO MATCH
    if( charDectected !=NULL)
    {
        arguments = arguments - 1;
    }
    ...
    else if ( pid > 0) // I am the parent process
    {
        ...
        if(charDectected!=NULL) // & character
        {
            //waitpid(pid,&status,WNOHANG); //Put process in background
        ...
        {
            waitpid(-1,&status,0); // wait for process to finish
        }
